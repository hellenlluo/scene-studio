...
def run(
    ...
    """Coupled optimisation over scale, pose and joint parameters. The core stage.

    Variables: per-object isotropic scale s_i, pose T_i in SE(3), per-joint axis,
    origin and limits. Objective:
        ...
        E = w_depth * E_depth    point-to-mesh, backprojected cloud vs posed mesh
          ...
          + w_sweep * E_sweep    over joints, over q across range, same         <- the novel term
    ...
    E_sweep is the expensive one — sample q at roughly 20 points per joint,
    collision-query each, cache aggressively, articulated objects only.
    ...
    respect SceneObject.is_pinned and Joint.is_pinned by holding those variables
    out of the free set rather than penalising them heavily.
